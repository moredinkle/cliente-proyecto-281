<template>
  <div class="VerCronogramaBenefico my-10">
    <div v-if="numActBeneficas > 0">
...
export default {
  ...
  methods: {
    mostrar() {
      axios
        .get(url + this.idProyecto + "/actividades-beneficas")
        .then((response) => {
          if (response.status == 200) {
            ...
            for (let i = 0; i < this.actividades.length; i++) {
              ...
              this.numActBeneficas = response.data.length;
            }
          }
        })
        .catch((error) => {
