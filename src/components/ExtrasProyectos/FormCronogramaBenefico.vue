              ...
              <v-card-text>
                <v-container>
                  <v-form ref="form" v-model="valid" lazy-validation>
                    ...
                    ></v-text-field>
                    <v-text-field
                      v-model="editedItem.beneficiario_actividad"
                      label="Beneficiario actividad"
                      required
                      filled
                      :counter="100"
                      :rules="nombre_beneficiarioRules"
                      clearable
                      clear-icon="mdi-close-circle"
                    ></v-text-field>
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { text: "Beneficiario", value: "beneficiario_actividad" },
  ...
  methods: {
    mostrar(){
      axios
        .get(url + this.idProyectoCreado +'/actividades-beneficas')
        .then((response) => {
        ...
        .catch((error) => {
          ...
          if(error.response.status == 400) alert("Error al guardar actividades ")
    ...
    deleteItemConfirm() {
      //version final
      axios
        .delete(url + 'actividades-beneficas/' + this.editedItem.id_actividad_benefica)
        .then((response) => {
          if (response.status == 200) {
            this.mostrar();
          }
        ...
        .catch((error) => {
          console.log(error.response);
          if(error.response.status == 400) alert("Error al eliminar actividad benefica")
          else alert("Error de servidor")
        });
      this.closeDelete();
    },
    ...
    save() {
      let data={
        ...
        beneficiario_actividad: this.editedItem.beneficiario_actividad
      }
      if (this.editedIndex > -1) {
        //editar
        axios
        .put(url + 'actividades-beneficas/' + this.editedItem.id_actividad_benefica, data)
        .then((response) => {
          if (response.status == 200) {
            this.mostrar();
          }
        ...
        .catch((error) => {
          console.log(error.response);
          if(error.response.status == 400) alert("Error al editar actividad benefica")
          else alert("Error de servidor")
        });
      } else {
        //crear nuevo
        axios
        .post(url + this.idProyectoCreado+'/actividades-beneficas', data)
        .then((response) => {
          if (response.status == 200) {
            this.mostrar();
          }
        ...
        .catch((error) => {
          console.log(error.response);
          if(error.response.status == 400) alert("Error al crear actividad benefica")
          else alert("Error de servidor")
        });
      }
      this.close();
